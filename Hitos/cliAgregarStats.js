      ...
      mdLinks(path.resolve(folderPath), options)
        .then(results => {
          if (options.stats) {
            ...
            console.log(chalk.bgHex('#FF6D28').bold('Total:  '), chalk.hex('#FF6D28')(total));
            console.log(chalk.bgHex('#FCE700').bold('Unique: '), chalk.hex('#FCE700')(unique));
            if (options.validate) {
              const broken = results.filter(result => result.status !== 200).length;
              console.log(chalk.bgHex('#EA047E').bold('Broken: '), chalk.hex('#EA047E')(broken));
            }
          } else {
